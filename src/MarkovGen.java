/*MarkovGenOrderM
 * twitterGen.train(dataNew);						
   twitterGen.calculateProb(markovOrder, false);		
   twitterGen.setData("Words", false);	
   generatedTweet = twitterGen.generate(false, period);
*/
...
public class MarkovGen<E> {
	...
	double[][] tempArr; // temporary array to hold the actual probabilities
	double random; // holds random number
	double sum1; // sum 1 and 2 hold the values that determines where the random number falls in
					// the transiton table
	double sum2;
	Random rand = new Random(); // new random objects
	Random rand2 = new Random();

	int[][] transCount; // this holds the transiiton table with the int counts before it is converted
						// into a trans table of probabilities
	int lengthOfSong; // changes whether the generated song is the same size as the midi file or of
						// size 20 for unit test 3
	...
	// generate song
	public ArrayList<E> generate(int length) { // E per now checks for period
		data = new ArrayList<E>(); // holds data during generation

		int randomOrderSelect = rand2.nextInt(numSymbols); // rand number to randomly pick first order used for each
															// song. This way, the beginning of the higher orders aren't
															// the exact same
		data.addAll(symbOrdersFound.get(randomOrderSelect)); // add the first order
		int g = symbOrdersFound.indexOf(symbOrdersFound.get(randomOrderSelect)); // know what column the order is in so
																					// we can go to corresponding
																					// probabilities

		lengthOfSong = length - orderOf; // if unit test one, just make the songs the smae length as the
													// midi file

		boolean stopSentence = false;
		for (int p = 0; p < lengthOfSong; p++) {
			random = rand.nextDouble(); // get random number
			sum1 = 0; // checking if random number is between sum1 and sum2, if so this is where the
						// next note is chosen
			sum2 = tempArr[g][0];
			...
			if (p > 0) { // doesn't go through first iteration because first
				...
				for (int o = 0; o < orderOf; o++) {
					v++;
					temp.add(data.get(v - 1)); // temp is now going to have the values of the most recent generation and
												// only the amount of symbols needed in each order. This is how the next
												// one will be chosen
				}
				g = symbOrdersFound.indexOf(temp); // find index number of latest order
				if ((g < 0) || (g >= symbOrdersFound.size()))
					g = 0;
				sum2 = tempArr[g][0]; // the bounds are moving to different numbers for the next number check (except
										// initial)
			}

			for (int l = 0; l < symbFound.size(); l++) {
				if (random >= sum1 && random < sum2) { // check if random number is between bounds that would select
														// symbol
				...
				if (l < symbFound.size() - 1)
					sum2 = sum2 + tempArr[g][l + 1]; // move bounds for sum2
				if (l == symbFound.size() - 1)
					sum2 = sum2 + tempArr[g][symbFound.size() - 1];
				// just some if statement check to check if all probabilities are 0 by chance
				// and it fails
				if ((sum2 == 0) && (l == symbFound.size() - 1))
					g = g - 2;
			}

			if (stopSentence)
				break; // break out of loop and end function if end of sentence
		}
		transTable.clear();
	...
	// this function adds to the temporary array the number of times a symbol is
	// found after the other
	// the numbers in each row are then divided by the symbol count to find the
	// probabilities

	void calculateProb(int orderNum, boolean printData) {
		symbFound = new ArrayList<E>(); // array list for symbols found
		int countT = 1;
		for (int h = 0; h < dataset.size() - 1; h++) { // This for loop simply gets the symbols found and also the count
														// to be able to initialize some arrays
			E nextInd = dataset.get(h + 1);
			E currInd = dataset.get(h);
			if (h == 0)
				symbFound.add(currInd);
		...
		int countP = 0;
		int indexOfOrder;
		int indexOf; // find index of the var after the order that was recorded so we know where to
						// add to the count in the transition table
		int holder = 0;
		...
		for (int i = orderOf - 1; i < dataset.size() - 1; i++) { // for loop to scroll through all of the data set
			...
			for (int d = 0; d < orderOf; d++) { // this gets the symbol orders
				indexOfOrder = d + countP; // when countP is added by 1 after, the next order will be different as it
											// pushes it by 1. This will eventually go through the whole song
				currRow.add(dataset.get(indexOfOrder)); // currRow is a temporary array that will eventuall be added to
														// a bigger transition table
				indexPlus1 = dataset.get(indexOfOrder + 1); // this gets us the index in symbolsFound of the symbol that
															// falls after the order. This will then add a 1
			}
			...
			if ((!foundOrder) && (i > orderOf - 1))
				transCount[holder + 1][indexOf] = transCount[holder + 1][indexOf] + 1;
			if ((foundOrder) || (foundOrder == false && i == orderOf - 1))
				transCount[holder][indexOf] = transCount[holder][indexOf] + 1;
		} // End of main for Loop
		tempArr = new double[symbOrdersFound.size()][symbFound.size()];
	}

	// this function sets the data from the temporary 2d array to multiple array
	// lists to ultimately
	...
	void setData(String dataType, boolean printOffOn) {
		...
		for (int j = 0; j < symbOrdersFound.size(); j++) { // nested loop to do math and ultimately set trassition table
															// up with actual probabilities
			ArrayList<Double> tempList = new ArrayList<Double>(); // create temporary array list to ultimately be added
																	// to trans table after values are divided by the
																	// sum
	...
	// This function prints the data. It is only called if the printData is true in
	// main
